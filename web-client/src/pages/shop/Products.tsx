...
import { getMyShopProducts, getCategories, createProduct, uploadProductImage, Product, Category, ProductColorStock, deleteProduct, updateProduct } from "../../api/productApi"; 
...
const Products: React.FC = () => {
    ...
    const filteredProducts = normalizedSearchTerm
        ? products.filter((product) => {
            const stock = normalizeStock(product.size_details);
            const stockText = Object.entries(stock)
                .flatMap(([color, sizes]) => [color, ...Object.keys(sizes)])
                .join(" ");
            const categoryText = product.categories?.map((category) => category.name).join(" ") || "";
            const colorText = product.colors?.join(" ") || product.available_colors?.join(" ") || "";
            ...
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="relative w-full md:max-w-xl">
                    ...
                    <input
                        ...
                        placeholder="Tìm sản phẩm theo tên, mã, thương hiệu, chất liệu, màu, size..."
                        className="w-full rounded-2xl border border-gray-200 bg-white py-3 pl-12 pr-4 text-sm font-semibold text-gray-700 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50"
                    />
                </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {filteredProducts.length > 0 ? paginatedProducts.map((p) => (
                            ...
                            <tr>
                                <td colSpan={4} className="py-10 text-center text-sm font-bold text-gray-400">
                            ...
                            <div className="lg:col-span-5 space-y-8">
                                ...
                                <div className="bg-gray-900 rounded-[24px] p-6 text-white">
                                    <div className="flex justify-between items-center mb-4">
                                        <label className="text-xs font-black text-gray-400 uppercase">Mau, kich thuoc & kho</label>
